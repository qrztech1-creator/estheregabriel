...
import { ArrowLeft, Save, ExternalLink, Copy, Eye, MessageCircle } from "lucide-react";
  ...
  const loadProposal = async () => {
    ...
    if (data) {
      ...
      setForm({
        ...
        client_instagram: (data as any).client_instagram || "",
        contract_value: Number((data as any).contract_value) || 0,
        contract_status: (data as any).contract_status || "proposal_sent",
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" asChild>
            <a href={`/proposta/${proposal.slug}`} target="_blank"><ExternalLink className="w-3.5 h-3.5 mr-1.5" /> Abrir</a>
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-card border border-border rounded-xl p-3 sm:p-4">
          ...
          <p className="text-lg font-bold">{(proposal as any).view_count || 0}</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-3 sm:p-4">
      ...
      <div className="bg-card border border-border rounded-xl p-4 sm:p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <Input value={form.client_email} onChange={e => setForm(f => ({ ...f, client_email: e.target.value }))} placeholder="email@exemplo.com" />
          </div>
          <div>
            <Label className="text-xs">Telefone</Label>
            <Input value={form.client_phone} onChange={e => setForm(f => ({ ...f, client_phone: e.target.value }))} placeholder="(27) 99999-9999" />
          </div>
          <div>
            <Label className="text-xs">Instagram</Label>
            <Input value={form.client_instagram} onChange={e => setForm(f => ({ ...f, client_instagram: e.target.value }))} placeholder="@instagram" />
          </div>
          <div>
            <Label className="text-xs">Valor do Contrato (R$)</Label>
            <Input type="number" value={form.contract_value} onChange={e => setForm(f => ({ ...f, contract_value: Number(e.target.value) }))} />
          </div>
          <div>
        ...
        <div>
          ...
          <Textarea
            ...
            placeholder="Observações sobre o casal, preferências, detalhes do contrato..."
            rows={4}
          />
        </div>
